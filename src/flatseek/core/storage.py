...
class RangeFile:
    ...
    2. ``read(n)`` issues one ``GET`` per 64 KB block, with
       ``Range: bytes=<start>-<end>``. Cached blocks are reused.
    3. A persistent ``httpx.Client`` with HTTP/2 is reused across all
       requests — single TCP/TLS connection, request multiplexing.
    4. The file size is probed lazily on first read (a small Range request
    ...
    """
    # 256 KB block size — optimal for large sequential reads (offset tables).
    # With 64 KB, a 1.6 MB offset table needs ~26 requests.
    # With 256 KB, it needs only ~7 requests.
    BLOCK_SIZE = 262144
    # Maximum concurrent range requests to fire at once. HTTP/2 multiplexing
    # handles batching these over a single TCP connection. 16 is a safe
    # balance between throughput and server stream limits.
    MAX_CONCURRENT = 16
    ...
    def __init__(self, url: str, *, timeout: float = 30.0,
        ...
        try:
            ...
            # h2 not installed — fall back to HTTP/1.1 with keep-alive
            self._client = httpx.Client(
                timeout=timeout,
                follow_redirects=True,
            )
        ...
        # True once we've seen a partial block (EOF boundary known)
        self._eof_hit = False
        # Thread pool for concurrent block fetching (exploits HTTP/2 multiplexing)
    ...
    def read(self, n: int = -1) -> bytes:
        ...
        For sequential large reads, fires all needed 64 KB block requests
        concurrently via a thread pool, exploiting HTTP/2 multiplexing over
        the single persistent TCP connection.
        """
        # Fast path: server didn't honor Range — we have the whole file.
        ...
        # Determine which blocks are needed for this read.
        start_pos = self._pos
        end_pos = min(self._pos + n, self._size or float("inf"))
        start_block = start_pos // self.BLOCK_SIZE
        end_block = (end_pos - 1) // self.BLOCK_SIZE + 1
        num_blocks = end_block - start_block

        if num_blocks <= 1:
            # Single block — synchronous fetch is fine (no overhead)
            chunk = self._fetch_block(start_block)
            if chunk is None or chunk == b"":
                self._pos = end_pos
                return b""
            block_offset = start_pos - start_block * self.BLOCK_SIZE
            take = min(len(chunk) - block_offset, n)
            self._pos += take
            return chunk[block_offset:block_offset + take]

        # Multiple blocks: fetch all concurrently, then splice from results.
        # Submit all futures at once — HTTP/2 multiplexes them over 1 TCP conn.
        futures: dict[int, concurrent.futures.Future] = {}
        for b_idx in range(start_block, end_block):
            if b_idx not in self._cache or len(self._cache[b_idx]) < self.BLOCK_SIZE:
                futures[b_idx] = self._executor.submit(self._fetch_block, b_idx)

        # Splice bytes from cached blocks + resolved futures.
        out = bytearray()
        pos = start_pos
        for b_idx in range(start_block, end_block):
            # Wait for this block's future if it was submitted
            if b_idx in futures:
                block_data = futures[b_idx].result()
                self._cache[b_idx] = block_data
            else:
                block_data = self._cache.get(b_idx, b"")
            if not block_data:
                break  # EOF

            b_start = b_idx * self.BLOCK_SIZE
            b_end = b_start + len(block_data)
            overlap_start = max(pos, b_start)
            overlap_end = min(pos + (end_pos - pos), b_end)
            if overlap_end > overlap_start:
                off_in_block = overlap_start - b_start
                take = overlap_end - overlap_start
                out.extend(block_data[off_in_block:off_in_block + take])
                pos += take
            if pos >= end_pos:
                break

        self._pos = start_pos + len(out)
        return bytes(out)
    ...
    def close(self) -> None:
        """Close the persistent HTTP client, thread pool, and clear cache."""
        self._cache.clear()
    ...
    def _fetch_block(self, block_idx: int) -> bytes | None:
        """Fetch a 64 KB block, with cache.

        If we already cached a partial block (e.g., the 1-byte block from
        the size probe), re-fetch the full 64 KB version.
        """
        cached = self._cache.get(block_idx)
        if cached is not None and len(cached) >= self.BLOCK_SIZE:
            return cached
        if self._size == 0:
            return b""
        ...
        # Retry on HTTP/2 stream closure (server pushed too many concurrent streams)
        for attempt in range(3):
            try:
                ...
                # Stream closed —短暂等待后重试
                import time as _time
                _time.sleep(0.1 * (attempt + 1))

        if resp.status_code == 200:
            # Server ignored Range — cache the full response
